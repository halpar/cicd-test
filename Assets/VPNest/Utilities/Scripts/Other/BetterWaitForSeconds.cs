...
public static class BetterWaitForSeconds
{
	...
	{
		WaitForSeconds instance = waitForSecondsPool.Count > 0 ? waitForSecondsPool.Pop() : new WaitForSeconds();
	...
	{
		WaitForSecondsRealtime instance = waitForSecondsRealtimePool.Count > 0 ? waitForSecondsRealtimePool.Pop() : new WaitForSecondsRealtime();
		instance.Initialize(seconds);
		return instance;
	}
